(() => {
  ...
  const sceneInfo = [
    // 0
    {
      ...
      objs: {
        ...
        profile_context: document
          .querySelector(".profile-img-canvas")
          .getContext("2d"),
        profileImg: [],
      },
      //Valus, Numbers
    ...
    // 1
    {
      ...
      objs: {
        ...
        main_message: document.querySelector(
          "#scroll-section-1 .main-message h2"
        ),
        project_div: document.querySelector("#scroll-section-1 .project"),
        message_div_automouse: document.querySelector(
          ".project-message.automouse"
        ),
        message_div_nas: document.querySelector(".project-message.nas-server"),
        project_automouse_title: document.querySelector(
          ".project-message-title.automouse"
        ),
        project_nas_title: document.querySelector(
          ".project-message-title.nas-server"
        ),
        ...
        project_context: document
          ...
          .getContext("2d"),
        imagesPath: [
          "./images/projects/automouse.png",
          "./images/projects/theia-code.png",
        ],
        projectImg: [],
  ...
  function setLayout() {
    ...
    if (window.innerWidth / beforeInnerHeight < 1920 / 1080) {
      sunset_translateX =
        -(1920 / 1080 - window.innerWidth / beforeInnerHeight) * 10 - 50;
    }
    ...
    sceneInfo[1].objs.project_canvas.style.transform = `translate3d(-50%, ${
      projectYtrans - 50
    }%, 0) scale(${section1_canvas_scale})`;
    sceneInfo[1].objs.message_div_automouse.style.width = `${
      1200 * section1_canvas_scale * project_message_width_Ratio
    }px`;
    sceneInfo[1].objs.message_div_nas.style.width = `${
      1200 * section1_canvas_scale * project_message_width_Ratio
    }px`;
    sceneInfo[1].objs.project_automouse_title.style.marginTop = `${
      beforeInnerHeight * sceneInfo[1].heightNum * 0.24
    }px`;
    sceneInfo[1].objs.project_nas_title.style.marginTop = `${
      beforeInnerHeight * sceneInfo[1].heightNum * 0.14
    }px`;
    sceneInfo[1].objs.slideArea.style.marginTop = `${
      beforeInnerHeight * sceneInfo[1].heightNum * 0.18
    }px`;
    ...
    sceneInfo[1].objs.loop_video.style.width = `${
      1200 * section1_canvas_scale * project_message_width_Ratio
    }px`;
    ...
    sceneInfo[2].objs.possible_activities.style.marginTop = `${
      beforeInnerHeight * sceneInfo[2].heightNum * activities_marginTop
    }px`;
  ...
  function imgChange(length, direction, projectName) {
    for (let i = 0; i < length; i++) {
      direction[i].src = `./images/projects/${projectName}/${projectName}-${
        i + 1
      }.jpg`;
    }
  }
  ...
  function playAnimation() {
    ...
    switch (currentScene) {
      case 0:
        objs.canvasDiv.style.opacity = calcValues(
      ...
      case 1:
        ...
        if (scrollRatio <= 0.18) {
          objs.project_canvas.style.opacity = calcValues(
            values.automouse_img_opacity_in,
            currentYOffset
          );
        } else if (scrollRatio <= 0.25) {
          objs.project_canvas.style.opacity = calcValues(
            values.automouse_img_opacity_down,
            currentYOffset
          );
        } else if (scrollRatio <= 0.36) {
          objs.project_canvas.style.opacity = calcValues(
            values.automouse_img_opacity_out,
            currentYOffset
          );
        } else if (scrollRatio <= 0.43) {
          objs.project_canvas.style.opacity = calcValues(
            values.nas_img_opacity_in,
            currentYOffset
          );
        } else if (scrollRatio <= 0.57) {
          objs.project_canvas.style.opacity = calcValues(
            values.nas_img_opacity_down,
            currentYOffset
          );
        } else {
          objs.project_canvas.style.opacity = calcValues(
            values.nas_img_opacity_out,
            currentYOffset
          );
        }
        if (scrollRatio <= 0.72) {
        ...
        if (scrollRatio <= 0.92) {
          objs.this_div.style.opacity = calcValues(
            values.this_in,
            currentYOffset
          );
        ...
        if (scrollRatio <= 0.97) {
          objs.this_div.style.backgroundColor = `rgba(255, 255, 255, ${calcValues(
            values.this_white_opacity,
            currentYOffset
          )})`;
          objs.container.style.backgroundColor = `rgba(${calcValues(
          ...
          sceneInfo[2].objs.jump_context.drawImage(
            sceneInfo[2].objs.jumpImg[0],
            0,
            0
          );
      ...
      case 2:
        ...
        if (scrollRatio <= 0.32) {
          objs.jump_canvas.style.opacity = calcValues(
            values.jump_opacity_down,
            currentYOffset
          );
          objs.jump_div.style.zIndex = 0;
        } else {
          objs.jump_canvas.style.opacity = calcValues(
            values.jump_opacity_out,
            currentYOffset
          );
        ...
        if (scrollRatio <= 0.22) {
          objs.main_message.style.opacity = calcValues(
            values.main_message_opacity_in,
            currentYOffset
          );
          objs.main_message.style.letterSpacing = `${calcValues(
            values.main_message_spacing,
            currentYOffset
          )}vw`;
        } else {
          objs.main_message_nothing.style.opacity = calcValues(
            values.main_message_nothing_opacity,
            currentYOffset
          );
          objs.main_message_upper.style.opacity = calcValues(
            values.main_message_upper_opacity,
            currentYOffset
          );
          objs.main_message_space.style.letterSpacing = `${calcValues(
            values.main_message_space_spacing,
            currentYOffset
          )}vw`;
          objs.main_message.style.opacity = calcValues(
            values.main_message_opacity_out,
            currentYOffset
          );
        }
        if (scrollRatio <= 0.33) {
          objs.main_message.style.transform = `translate3d(0, ${calcValues(
            values.main_message_translateY_middle,
            currentYOffset
          )}vw, 0)`;
        } else {
          objs.main_message.style.transform = `translate3d(0, ${calcValues(
            values.main_message_translateY_out,
            currentYOffset
          )}vw, 0)`;
        }
        ...
        break;
      case 3:
        objs.rowDiv.style.transform = `translate3d(${calcValues(
          values.rowDiv_translateX,
          currentYOffset
        )}%, 0, 0)`;
        if (scrollRatio <= 0.22) {
          // in
    ...
    if (!enterNewScene) {
      if (currentScene === 0 || currentScene === 3) {
        ...
        if (currentScene === 0 && sequence >= 370) {
          ...
          objs.canvas.style.transform = `translate3d(${
            -50 + (sequence - 370) / 2
          }%, ${canvas_transY - (sequence - 370)}%, 0) scale(${
            1 + (sequence - 370) / 30
          })`;
        }
      ...
      if (currentScene === 1) {
        ...
        if (section2_ratio < 0.87) {
          objs.jump_context.clearRect(
            0,
            0,
            objs.jump_canvas.width,
            objs.jump_canvas.height
          );
        } else {
  ...
  function slideElement(slideImgs) {
    if (currentScene === 1) {
      ...
      if (beforeInnerHeight < (imgWidth / 1000) * 540 + 50 + 50 + 30) {
        slideArea.style.maxWidth = `${
          (beforeInnerHeight / 540) * 1000 * 0.7
        }px`;
